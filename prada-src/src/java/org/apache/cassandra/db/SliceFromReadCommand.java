...
public class SliceFromReadCommand extends ReadCommand
{
    ...
    public SliceFromReadCommand(String keyspaceName, ByteBuffer key, String cfName, long timestamp, SliceQueryFilter filter, InetAddress from)
    {
        super(keyspaceName, key, cfName, timestamp, Type.GET_SLICES, from);
        this.filter = filter;
    }
    ...
    {
        ReadCommand readCommand = new SliceFromReadCommand(ksName, key, cfName, timestamp, filter, from);
        readCommand.setDigestQuery(isDigestQuery());
        return readCommand;
    }

    public ReadCommand copy(String newKsName)
    {
        ReadCommand readCommand = new SliceFromReadCommand(newKsName, key, cfName, timestamp, filter, from);
        readCommand.setDigestQuery(isDigestQuery());
        return readCommand;
    }
    ...
    public Row getRow(Keyspace keyspace)
    {
        ...
        // Check if Indirection Information exists
        Row IndRow;
        IndRow = IndirectionReadSupport.getIndirectionRow(keyspace, new QueryFilter(dk, cfName, filter, timestamp));
        if (IndRow != null && IndRow.cf.getColumnCount() > 1)
            return IndRow;
    ...
    {
        return new SliceFromReadCommand(ksName, key, cfName, timestamp, newFilter, from);
    }
...
class SliceFromReadCommandSerializer implements IVersionedSerializer<ReadCommand>
{
    ...
    public void serialize(ReadCommand rm, ByteBuffer superColumn, DataOutput out, int version) throws IOException
    {
        ...
        ByteBufferUtil.writeWithShortLength(ByteBuffer.wrap(rm.from.getAddress()), out);
    }
    ...
    public ReadCommand deserialize(DataInput in, int version) throws IOException
    {
        ...
        InetAddress from = InetAddress.getByAddress(ByteBufferUtil.readWithShortLength(in).array());

        ReadCommand command = new SliceFromReadCommand(keyspaceName, key, cfName, timestamp, filter, from);
    ...
    public long serializedSize(ReadCommand cmd, ByteBuffer superColumn, int version)
    {
        ...
        size += sizes.sizeof((short) cmd.from.getAddress().length) + cmd.from.getAddress().length;
