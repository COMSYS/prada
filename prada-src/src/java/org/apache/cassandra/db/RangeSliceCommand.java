...
import org.apache.cassandra.annotation.IndirectionReadSupport;
...
public class RangeSliceCommand extends AbstractRangeCommand implements Pageable
{
    ...
    public AbstractRangeCommand forKeyspace(String keyspace)
    {
        return new RangeSliceCommand(keyspace,
                                     columnFamily,
                                     timestamp,
    ...
    public List<Row> executeLocally()
    {
        ...
        if (origin != null)
        {
            keyspace += IndirectionSupport.DataKeyspacePostfix;
        }
        ...
        if (origin == null)
        {
            // Check for and add Indirection Rows
            result.addAll(IndirectionReadSupport.getIndirectionReferenceRows(keyspace, columnFamily, exFilter, rowFilter));
        }
...
class RangeSliceCommandSerializer implements IVersionedSerializer<RangeSliceCommand>
{
    public void serialize(RangeSliceCommand sliceCommand, DataOutput out, int version) throws IOException
    {
        ...
        out.writeInt(sliceCommand.originId);
        if (sliceCommand.origin == null)
        {
            out.writeShort((short)0);
        }
        else
        {
            ByteBufferUtil.writeWithShortLength(ByteBuffer.wrap(sliceCommand.origin.getAddress()), out);
        }
    }
    ...
    public RangeSliceCommand deserialize(DataInput in, int version) throws IOException
    {
        ...
        RangeSliceCommand c = new RangeSliceCommand(keyspace, columnFamily, timestamp, predicate, range, rowFilter, maxResults, countCQL3Rows, isPaging);
        c.originId = in.readInt();
        int originLen = in.readShort();
        if (originLen > 0)
        {
            c.origin = InetAddress.getByAddress(ByteBufferUtil.read(in, originLen).array());
        }
    ...
    public long serializedSize(RangeSliceCommand rsc, int version)
    {
        ...
        size += TypeSizes.NATIVE.sizeof(rsc.originId);
        if (rsc.origin == null)
        {
        ...
        {
            size += TypeSizes.NATIVE.sizeof((short) rsc.origin.getAddress().length) + rsc.origin.getAddress().length;
        }
        return size;
    }
}
