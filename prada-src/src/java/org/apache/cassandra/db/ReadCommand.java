...
public abstract class ReadCommand implements IReadCommand, Pageable
{
    ...
    protected ReadCommand(String ksName, ByteBuffer key, String cfName, long timestamp, Type cmdType, InetAddress from)
    {
        this.ksName = ksName;
        this.key = key;
        this.cfName = cfName;
    ...
    protected ReadCommand(String ksName, ByteBuffer key, String cfName, long timestamp, Type cmdType)
    {
        ...
        this.timestamp = timestamp;
        this.commandType = cmdType;
        this.from = FBUtilities.getBroadcastAddress();
    }

    public static ReadCommand create(String ksName, ByteBuffer key, String cfName, long timestamp, IDiskAtomFilter filter, InetAddress from)
    {
        if (filter instanceof SliceQueryFilter)
            return new SliceFromReadCommand(ksName, key, cfName, timestamp, (SliceQueryFilter)filter, from);
        else
            return new SliceByNamesReadCommand(ksName, key, cfName, timestamp, (NamesQueryFilter)filter, from);
    }
    ...
    public abstract ReadCommand copy(String newKsName);
    ...
    public void serialize(ReadCommand command, DataOutput out, int version) throws IOException
    {
        ...
        if (version < MessagingService.VERSION_20)
        {
            ...
            if (metadata.cfType == ColumnFamilyType.Super)
            {
                ...
                newCommand = ReadCommand.create(command.ksName, command.key, command.cfName, command.timestamp, scFilter.updatedFilter, command.from);
