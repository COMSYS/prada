...
public class SliceByNamesReadCommand extends ReadCommand
{
    ...
    public SliceByNamesReadCommand(String keyspaceName, ByteBuffer key, String cfName, long timestamp, NamesQueryFilter filter, InetAddress from)
    {
        super(keyspaceName, key, cfName, timestamp, Type.GET_BY_NAMES, from);
        this.filter = filter;
    }
    ...
    {
        ReadCommand readCommand= new SliceByNamesReadCommand(ksName, key, cfName, timestamp, filter, from);
    ...
    {
        ReadCommand readCommand= new SliceByNamesReadCommand(newKsName, key, cfName, timestamp, filter, from);
        readCommand.setDigestQuery(isDigestQuery());
        return readCommand;
    }
    ...
    public ReadCommand deserialize(DataInput in, int version) throws IOException
    {
        ...
        if (version < MessagingService.VERSION_20)
        {
            ...
            if (filter instanceof NamesQueryFilter)
                command = new SliceByNamesReadCommand(keyspaceName, key, cfName, timestamp, (NamesQueryFilter)filter, from);
            else
                command = new SliceFromReadCommand(keyspaceName, key, cfName, timestamp, (SliceQueryFilter)filter, from);
        }
        else
        {
            NamesQueryFilter filter = NamesQueryFilter.serializer.deserialize(in, version, metadata.comparator);
            InetAddress from = InetAddress.getByAddress(ByteBufferUtil.readWithShortLength(in).array());
            command = new SliceByNamesReadCommand(keyspaceName, key, cfName, timestamp, filter, from);
        }
