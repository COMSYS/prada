...
public class RangeSliceResponseResolver implements IResponseResolver<RangeSliceReply, Iterable<Row>>
{
    ...
    public final Map<InetAddress, Part> parts;
    ...
    public Iterable<Row> resolve()
    {
        ...
        // If the row belongs to the reference keyspace, we have to set the indirection flag in the new row by using maybeTransformToIndirectionRow()
        while (iter.hasNext())
        {
            Row row = iter.next();
            if (row != null && row.cf != null && row.cf.metadata().ksName.endsWith(IndirectionSupport.ReferenceKeyspacePostfix) && !row.indirection)
                resolvedRows.add(new Row(row.key, row.cf, true));
            else
    ...
    public void preprocess(MessageIn<RangeSliceReply> message)
    {
        RangeSliceReply recvd = message.payload;
        InetAddress addr = recvd.origin == null ? message.from : recvd.origin;
        Part p = parts.get(addr);
        synchronized (p)
        {
            p.count -= 1 + (recvd.origin == null ? (Integer.MAX_VALUE - recvd.count) : 0);
            p.rows.addAll(recvd.rows);
            if (p.count <= 0)
            {
                MessageIn<RangeSliceReply> msg = MessageIn.create(addr, new RangeSliceReply(new ArrayList<>(p.rows)), message.parameters, message.verb, message.version);
                responses.add(msg);
            }
        }
    }
    ...
    private static class Part
    {
        ...
        public Part()
        {
            ...
            rows = new TreeSet<>(comparator);
        }

        private static Comparator<Row> comparator = new Comparator<Row>() {
            @Override
