...
public class RowDigestResolver extends AbstractRowResolver
{
    ...
    public Row resolve() throws DigestMismatchException
    {
        ...
        // If the row belongs to the reference keyspace, we have to set the indirection flag in the new row
        if (data != null && data.metadata().ksName.endsWith(IndirectionSupport.ReferenceKeyspacePostfix))
            return new Row(key, data, true);
        else
