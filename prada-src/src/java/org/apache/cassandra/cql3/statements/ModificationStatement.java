...
public abstract class ModificationStatement implements CQLStatement, MeasurableForPreparedCache
{
    ...
    private double updateLoadCache(ByteBuffer key, ColumnFamily cf)
    {
        double size = cf.dataSize();
        Token tk = StorageService.getPartitioner().getToken(key);
        List<InetAddress> naturalEndpoints = StorageService.instance.getNaturalEndpoints(keyspace(), tk);
        Collection<InetAddress> pendingEndpoints = StorageService.instance.getTokenMetadata().pendingEndpointsFor(tk, keyspace());
        InetAddress ownAddr = FBUtilities.getBroadcastAddress();
        for (InetAddress target : Iterables.concat(naturalEndpoints, pendingEndpoints))
        {
            if (!ownAddr.equals(target))
                LoadBroadcaster.instance.updateLoadCache(target, size);
        }
        return size;
    }
    ...
    throws RequestExecutionException, RequestValidationException
    {
        ...
        double totalSize = 0;
        for (IMutation mut : mutations)
        {
            for (ColumnFamily cf : mut.getColumnFamilies())
                totalSize += updateLoadCache(mut.key(), cf);
        }
        queryState.setSizeDiff(totalSize);
    ...
    throws RequestExecutionException, RequestValidationException
    {
        ...
        if (result == null)
        {
            queryState.setSizeDiff(updateLoadCache(key, updates));
        }
    ...
    public static abstract class Parsed extends CFStatement
    {
        ...
        public Constraints annotations;
        ...
        {
            this(name, attrs, conditions, ifNotExists, new Constraints());
        }
        protected Parsed(CFName name, Attributes.Raw attrs, List<Pair<ColumnIdentifier, Operation.RawUpdate>> conditions, boolean ifNotExists, Constraints annotations)
        {
            super(name);
            this.attrs = attrs;
